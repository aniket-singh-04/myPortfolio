...
export default function Certificates() {
  ...
  return (
    <section id="certificates" className="section-container relative bg-surface-50/50 dark:bg-surface-900/50 border-y border-surface-200 dark:border-surface-800">
      ...
      {profileCertificates.length === 0 ? (
        <div className="mx-auto max-w-3xl rounded-3xl border border-dashed border-surface-300 bg-white p-8 text-center shadow-sm dark:border-surface-700 dark:bg-surface-800">
          ...
          <h3 className="mb-3 text-2xl font-display font-bold text-surface-900 dark:text-white">
            Verified certificates will be added here soon.
          </h3>
          <p className="mx-auto mb-6 max-w-xl text-sm leading-relaxed text-surface-600 dark:text-surface-400">
            For now, the strongest overview of my current background is in the education and project sections, plus my resume.
          </p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          <div key={cert.title} className="bg-white dark:bg-surface-800 p-6 rounded-2xl border border-surface-200 dark:border-surface-700 shadow-sm hover:shadow-md hover:border-primary-400/50 transition-all flex flex-col h-full group cursor-pointer">
            ...
            <div className="mt-auto pt-4 border-t border-surface-100 dark:border-surface-700 flex items-center justify-between">
              <span className="text-xs font-medium text-surface-400">{cert.date}</span>
              <button
                type="button"
      ...
      {selectedCert && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div
            className="relative z-10 w-full max-w-lg bg-white dark:bg-surface-800 rounded-2xl shadow-2xl p-6 sm:p-8"
          >
            <button
              type="button"
              onClick={() => setSelectedCert(null)}
            ...
            <div className="text-center">
              <h3 className="text-2xl font-bold font-display text-surface-900 dark:text-white mb-2">{selectedCert.title}</h3>
              <p className="text-surface-600 dark:text-surface-300 text-sm mb-6">Issued by <span className="font-semibold text-surface-900 dark:text-white">{selectedCert.issuer}</span> on {selectedCert.date}</p>
              ...
              {selectedCert.credentialId && (
                <div className="mb-6 flex flex-col items-center rounded-xl border border-surface-200 bg-surface-50 p-4 dark:border-surface-700 dark:bg-surface-900">
                  <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-surface-500">Credential ID</p>
                  <p className="font-mono font-bold text-surface-900 dark:text-white">{selectedCert.credentialId}</p>
                </div>
              )}
